...
namespace HanIlCNS
{
    ...
    {
        public List<OutDetailData> CreateOutDetailData(DataTable dt)
        {
            List<OutDetailData> data = new List<OutDetailData> ();
            ...
            foreach(DataRow row in dt.Rows)
            {
                ...
                OutDetailData oneData = new OutDetailData ();
                oneData.no = row["no"].ToString();
                oneData.productname = row["productname"].ToString();
                if (string.IsNullOrEmpty(row["outqty"].ToString()))
                    oneData.outqty = null;
                else
                    oneData.outqty = double.Parse(row["outqty"].ToString());
                oneData.outnote = row["outnote"].ToString();
